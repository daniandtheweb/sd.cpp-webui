...
with gr.Blocks() as convert_block:
    ...
    with gr.Row():
        with gr.Column():
            model_type = gr.Dropdown(label="Model Type", choices=MODELS,
                                     interactive=True,
                                     value="Stable-Diffusion")
            model_type.input(model_choice, inputs=[model_type],
    ...
    with gr.Row():
        ...
        with gr.Column():
            with gr.Row():
                quant_type = gr.Dropdown(label="Type", choices=QUANTS,
                                         value="f32")
    ...
    # Interactive Bindings
    convert_btn.click(convert, inputs=[model, model_dir_txt, quant_type,
                                       gguf_name, verbose], outputs=[result])
